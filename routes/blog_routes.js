...
function blog_routing (app) {
    ...
    app.get("/blog/create/:name", (req,res) => {
        if(req.verified && req.username == req.params.name) {
            res.render("blog", {writer : req.params.name, title : "", blog : "", sub_title : "", error : false,edit : false})
        } else {
            res.redirect("/403");
        }
    })
    ...
    app.post("/blog/del_new", async (req,res) => {
        if(req.verified && req.username == req.params.name) {
            ...
            /// deleting the old blog
                ...
                if(blog.length != 0) {
                    res.render("blog", {title : req.body.title,writer:user,sub_title:req.body.sub_title,blog : req.body.content, error : true, edit : false})
                } else {
    ...
    app.post("/blog/create/:name", async (req,res) => {
        if(req.verified && req.username == req.params.name) {
            ...
            const blog = await find(blogs, ["writer","title"], [re_1,re_2]);
            if(blog.length !== 0) {
                res.render("blog", {title : req.body.title,writer:req.params.name,sub_title:req.body.sub_title,blog : req.body.content, error : true, edit : false})
            } else {
                let file = req.files.display;
                let name = file.name.split(" ").join("");
                console.log(name);
